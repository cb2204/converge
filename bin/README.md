...
|---|---|---|
| `cvg init` | native workspace templates | creates the canonical tracked `.cvg/gate.yaml` and `cvg/{brain,docs,sketch,tasks,execution,loop,receipts}` control plane without clobbering existing files; pass prompts are NOT copied (each pass skill ships its own at `references/pass-prompt.md`); idempotent `CVG_INIT=OK\|UNCHANGED`. |
