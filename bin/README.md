...
|---|---|---|
| `cvg init` | native workspace templates | creates the canonical tracked `.cvg/gate.yaml` and `cvg/{brain,docs,swimlanes,tasks,execution,loop,receipts}` control plane without clobbering existing files; pass prompts are NOT copied (each pass skill ships its own at `references/pass-prompt.md`); idempotent `CVG_INIT=OK\|UNCHANGED`. |
...
| `cvg next [--lane FULL\|NORMAL\|FAST]` | `evidence-to-next-pass/scripts/next-pass.sh` | **the descent conductor** — derives which pass is current and which comes next from workspace evidence (no stored state), prints the evidence board, the owning skill and its installed steering prompt (`skills/<pass-skill>/references/pass-prompt.md`), and the pass's closing gate. Read-only and instant (presence probes, never gate execution); evidence presence is not a verdict — the gates stay authoritative. Same engine's `pre N`/`post N` hooks enforce order fail-closed. It also names the optional teaching companion (`cvg lesson`) wherever a pass has closed — advisory, never sequenced, and never ahead of the token. Proven: 27-check hermetic suite. `NEXT_PASS=0-8\|DONE`. |
| `cvg lesson [<file>] [--immutable <path>...]` | `pass-to-lesson/scripts/check-lesson.sh` | **the teaching companion's gate** — optional, invocable after ANY pass closes, and it never blocks the descent. Converge delegates the writing of every artifact but not the *understanding* of it, so this gates the lesson a pass earned: all seven sections by exact name (a `## Notes on why this pass exists` decoy does not count), *what breaks downstream* on every `### <component>` block, a non-empty decisions table, 3–5 check-yourself questions, and well-formed teaching-mode sections when the lesson declares `modes:`. `--immutable <taught-artifact>...` proves teaching changed nothing it taught (clean `git status --porcelain`; outside a repo it warns and skips rather than crashing). Discovery finds `cvg/docs/lessons/` then legacy `docs/lessons/`; lessons **accumulate by design**, so >1 is a usage error naming them all — newest-wins was rejected because git does not preserve mtimes, and a gate that picks its own target non-deterministically is not a gate. `CHECK_LESSON=PASS\|FAIL\|USAGE_ERROR`. |
...
`cvg/docs/adrs/` then `docs/adrs/` (Pass 2), `cvg/swimlanes/` then `swimlanes/`
...
evidence-to-next-pass surface; `lesson` **shipped** as the pass-to-lesson
surface — the last skill in the package that had no CLI door.)
