...
Conform `raw.*` to a silver layer — dedup by business signature, pin types, and set
the UTC grain — so that gold models build on clean, deterministic, tz-correct data.

## Proves (acceptance criteria — Given/When/Then; no evals)

- Given raw records with duplicate business signatures, when conform runs, then each signature appears once in silver.
...
- the dedup-by-signature model and its uniqueness check.
