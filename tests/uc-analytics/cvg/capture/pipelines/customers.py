"""Capture — customers off the Postgres WAL into raw.customers.
...
straight off a replication slot scoped to `public.customers` via a
publication, and lands one change-record row per event into
`raw.customers` in the frozen seam shape: customer_id / _lsn / _op /
_captured_at / _source_committed_at.

The read is scoped to `public.customers` only, via the dedicated
...
SLOT = "customers_capture_slot"
PUBLICATION = "customers_pub"

_COL_RE = re.compile(r"(\S+)\[([^\]]+)\]:")
_OP_RE = re.compile(r"^table public\.customers: (INSERT|UPDATE|DELETE):")
...
def bootstrap() -> None:
    ...
    _run_psql(ADMIN_USER, "GRANT SELECT, INSERT ON raw.customers TO capture_reader;")
    _run_psql(ADMIN_USER, "GRANT SELECT ON public.customers TO capture_reader;")

    pub_exists = _run_psql(
        ADMIN_USER, f"SELECT 1 FROM pg_publication WHERE pubname = '{PUBLICATION}';"
    ).strip()
    if not pub_exists:
        _run_psql(ADMIN_USER, f"CREATE PUBLICATION {PUBLICATION} FOR TABLE public.customers;")
...
def fetch_and_land() -> int:
    ...
    them into raw.customers. Returns the number of change-records landed."""
    raw = _run_psql(
        CAPTURE_USER,
    ...
    for line in raw.splitlines():
        ...
        if commit_m:
            ...
            for pending_lsn, pending_data in pending_by_xid.pop(xid, []):
                ...
                values.append(
                    "("
                    f"{_sql_literal(cols, 'customer_id')}, "
                    f"{_sql_literal(cols, 'full_name')}, "
                    f"{_sql_literal(cols, 'email')}, "
                    f"{_sql_literal(cols, 'country')}, "
                    f"{_sql_literal(cols, 'city')}, "
                    f"{_sql_literal(cols, 'segment')}, "
    ...
    insert_sql = (
        "INSERT INTO raw.customers (customer_id, full_name, email, country, "
        "city, segment, created_at, _lsn, _op, _captured_at, "
...
if __name__ == "__main__":
    landed = run()
    print(f"landed {landed} change-record(s) into raw.customers")
