"""Capture steel thread — orders off the Postgres WAL into raw.orders.
...
straight off a replication slot scoped to `public.orders` via a publication,
and lands one change-record row per event into `raw.orders` in the frozen
seam shape: order_id / _lsn / _op / _captured_at / _source_committed_at.

The read is scoped to `public.orders` only, via the dedicated
...
SLOT = "orders_capture_slot"
PUBLICATION = "orders_pub"

_COL_RE = re.compile(r"(\S+)\[([^\]]+)\]:")
_OP_RE = re.compile(r"^table public\.orders: (INSERT|UPDATE|DELETE):")
...
def bootstrap() -> None:
    ...
    _run_psql(ADMIN_USER, "GRANT SELECT, INSERT ON raw.orders TO capture_reader;")

    pub_exists = _run_psql(
        ADMIN_USER, f"SELECT 1 FROM pg_publication WHERE pubname = '{PUBLICATION}';"
    ).strip()
    if not pub_exists:
        _run_psql(ADMIN_USER, f"CREATE PUBLICATION {PUBLICATION} FOR TABLE public.orders;")
...
def fetch_and_land() -> int:
    ...
    them into raw.orders. Returns the number of order change-records landed."""
    raw = _run_psql(
        CAPTURE_USER,
        ...
        if commit_m:
            ...
            for pending_lsn, pending_data in pending_by_xid.pop(xid, []):
                ...
                values.append(
                    ...
                    f"{_sql_literal(cols, 'quantity')}, "
                    f"{_sql_literal(cols, 'unit_price')}, "
                    f"{_sql_literal(cols, 'total_amount')}, "
                    f"{_sql_literal(cols, 'status')}, "
                    f"{_sql_literal(cols, 'ordered_at')}, "
    ...
    insert_sql = (
        ...
        "unit_price, total_amount, status, ordered_at, _lsn, _op, _captured_at, "
...
if __name__ == "__main__":
    landed = run()
    print(f"landed {landed} change-record(s) into raw.orders")
