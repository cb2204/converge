"""Capture — payments off the Postgres WAL into raw.payments.
...
straight off a replication slot scoped to `public.payments` via a
publication, and lands one change-record row per event into
`raw.payments` in the frozen seam shape: payment_id / _lsn / _op /
_captured_at / _source_committed_at.

The read is scoped to `public.payments` only, via the dedicated
...
SLOT = "payments_capture_slot"
PUBLICATION = "payments_pub"

_COL_RE = re.compile(r"(\S+)\[([^\]]+)\]:")
_OP_RE = re.compile(r"^table public\.payments: (INSERT|UPDATE|DELETE):")
...
def bootstrap() -> None:
    ...
    _run_psql(ADMIN_USER, "GRANT SELECT, INSERT ON raw.payments TO capture_reader;")
    _run_psql(ADMIN_USER, "GRANT SELECT ON public.payments TO capture_reader;")

    pub_exists = _run_psql(
        ADMIN_USER, f"SELECT 1 FROM pg_publication WHERE pubname = '{PUBLICATION}';"
    ).strip()
    if not pub_exists:
        _run_psql(ADMIN_USER, f"CREATE PUBLICATION {PUBLICATION} FOR TABLE public.payments;")
...
def fetch_and_land() -> int:
    ...
    them into raw.payments. Returns the number of change-records landed."""
    raw = _run_psql(
        CAPTURE_USER,
    ...
    for line in raw.splitlines():
        ...
        if commit_m:
            ...
            for pending_lsn, pending_data in pending_by_xid.pop(xid, []):
                ...
                values.append(
                    "("
                    f"{_sql_literal(cols, 'payment_id')}, "
                    f"{_sql_literal(cols, 'order_id')}, "
                    f"{_sql_literal(cols, 'method')}, "
                    f"{_sql_literal(cols, 'amount')}, "
                    f"{_sql_literal(cols, 'status')}, "
                    f"{_sql_literal(cols, 'paid_at')}, "
    ...
    insert_sql = (
        "INSERT INTO raw.payments (payment_id, order_id, method, amount, "
        "status, paid_at, _lsn, _op, _captured_at, "
...
if __name__ == "__main__":
    landed = run()
    print(f"landed {landed} change-record(s) into raw.payments")
